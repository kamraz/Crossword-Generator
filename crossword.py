...
class Crossword_Solver:
    ...
    def solve(self, crossword: Crossword):
        solutions = self.modified_bfs_solve(crossword)
        if len(solutions) == 0:
    ...
    def modified_bfs_solve(self, crossword):
        "solve the crossword puzzle using breadth first search"
        queue = [crossword]
        a = 0
        print("-" * 20)
...
def find_crosswords():
    word_list = load_word_list("peter-broda-wordlist__scored.txt")
    word_list.append(["cardigan".upper(), 100])
    word_list.append(["fifth".upper(), 100])
    word_list.append(["townsend".upper(), 100])
    word_list.append(["fasting".upper(), 100])

    banned_words = ["ARGURNEY"]
    for entry in word_list:
        if entry[0] in banned_words:
            word_list.remove(entry)

    done = False
    while not done:
        necessary_blockers = [[1, 0], [2, 7], [7, 0]]

        # Generate a random number of other blockers
        other_blockers = []
        for i in range(random.randint(6, 15)):
            other_blockers.append([random.randint(1, 7), random.randint(1, 7)])

        other_blockers = [b for b in other_blockers if b[0] != 2 and b[0] != 5]

        print(other_blockers)
        crossword = Crossword(
            8,
            8,
            necessary_blockers + other_blockers,
            word_list,
        )
        crossword.get_slot_sizes()
        print(crossword.get_slot_sizes())
        print(crossword)
        crossword.place_word("cardigan".upper(), 0, 0, "across")
        crossword.place_word("fifth".upper(), 2, 0, "down")
        crossword.place_word("townsend".upper(), 5, 0, "across")
        crossword.place_word("fasting".upper(), 2, 0, "across")
        print(crossword)

        done = crossword.brute_force_add_words()
...
def main():
    ...
    necessary_blockers = [[1, 0], [1, 1], [2, 0], [6, 1], [6, 2], [7, 4]]
    optional_blockers = [
        [1, 6],
        [2, 6],
        [2, 5],
